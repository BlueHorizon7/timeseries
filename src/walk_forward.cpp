...
WalkForwardResult run_walk_forward(
    ...
    if (parameters.formation_size < 20) {
        throw std::invalid_argument(
            "Walk-forward formation size must be at least 20"
        );
    }

    if (parameters.test_size < 2) {
        throw std::invalid_argument(
            "Walk-forward test size must be at least two"
        );
    }

    if (parameters.formation_size >= data.size()) {
        throw std::invalid_argument(
            "Formation size must leave observations for testing"
        );
    }
    ...
    if (parameters.formation_size <=
        parameters.strategy.hedge_ratio_window) {

        throw std::invalid_argument(
            "Formation size must exceed hedge ratio window"
        );
    }
    ...
    if (parameters.formation_size <=
        ...
        throw std::invalid_argument(
            "Formation size must exceed z-score window"
        );
    }

    WalkForwardResult result;

    const std::size_t agg_start =
        parameters.formation_size;

    const std::size_t agg_size =
        data.size() - agg_start;
    ...
    /*
        Failed formation folds remain flat.
    */
    std::vector<int>
        agg_signals(
            agg_size,
    ...
    for (std::size_t i = agg_start;
         i < data.size();
         ++i) {

        agg_timestamps.push_back(
            data[i].timestamp
        );
    ...
    while (test_begin < data.size()) {
        ...
        */
        ...
        const std::size_t formation_end =
            test_begin;
        ...
        /*
            ...
            The cointegration/ADF configuration is evaluated
            independently on this fold's formation sample.

            Therefore AIC/BIC selection never sees OOS data.
        */
        ...
        if (formation_passed) {
            ...
            /*
                Include one extra observation so the final
                signal of this fold can be extracted for
                the interval:
            ...
            */
            ...
            /*
                Stitch signals and hedge ratios into the
                unified OOS timeline.

                Timestamps are the source of truth.
            */
            for (std::size_t t = test_begin;
                 t < test_end;
                 ++t) {
                ...
                */
                ...
                if (hr_it !=
                    ...
                    *hr_it == current_ts) {
                    ...
                    agg_hedge_ratios[out_idx] =
                        research.hedge_ratios[idx];
                }

                /*
                    Signal at t controls t -> t+1.
                */
                if (t + 1 < data.size()) {

                    const std::int64_t next_ts =
                        data[t + 1].timestamp;
    ...
    /*
        One unified OOS backtest.

        This is the authoritative aggregate result.
    */
    if (agg_size >= 2) {
